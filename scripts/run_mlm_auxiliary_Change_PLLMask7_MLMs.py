...
def calculate_word_level_scores(tokens, scores): #実装終了
    ...
    for i, token in enumerate(tokens):
        ...
        if (not(token.startswith('Ġ')) or token.startswith('##')) and i != 0 and token not in [",", "."]: #サブワード！
            ...
            # print("if3",token)
            word_scores.append(sum(ws)) #word Mask 用　今回のコードだけの処理
            words.append("".join(w))
...
def calculate_word_level_scores_forWeightWord(tokens, scores): #実装終了
    ...
    for i, token in enumerate(tokens):
        ...
        if (not(token.startswith('Ġ')) or token.startswith('##')) and i != 0 and token not in [",", "."]: #サブワード！
            ...
            # print("if3",token)
            word_scores.append(sum(ws)/len(ws)) #word Mask 用　今回のコードだけの処理
            words.append("".join(w))
            token_to_word.append(t_to_w)
...
def masking_probability(score, min_score, max_score, mean_score,mlm_probability):
    # 例: スコアが平均より低い場合、確率を高める v5
    prob = ((score - min_score) / (max_score-min_score))   # 0.15はデフォルトのマスキング確率
...
def mask_tokens_PRP(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word_to_phrase_, token_to_phrase,token_to_word, mlm_list, wordscores, phrase_scores, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    phrase_scores.insert(0, 0)#<s>
    phrase_scores.append(0)


    phrase_scores_ = []
    for i in phrase_scores:
        phrase_scores_.append(i * -1)

    min_score = min(phrase_scores_)
    max_score = max(phrase_scores_)
    mean_score = sum(phrase_scores_)/len(phrase_scores_)

    probability_matrix = []
    for score in phrase_scores_:
    ...
    masked_indices_ = []
    for token, mask in zip(token_to_phrase, masked_indices[0]):
        if type(token)==list:
...
def mask_tokens_PRP_WordWeight(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word_to_phrase_, token_to_phrase,token_to_word, mlm_list, wordscores, args) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. """
    mlm_mask = task_info[task_name]["mlm_mask"]# default 0.8
    mlm_random = task_info[task_name]["mlm_random"]# default 0.5
    mlm_probability = task_info[task_name]["mlm_probability"]#args.word_mask_probability[task_index]
...
def mask_tokens_PRP_PMI(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word_to_phrase_, token_to_phrase,token_to_word, mlm_list, args) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. """
    mlm_mask = task_info[task_name]["mlm_mask"]# default 0.8
    mlm_random = task_info[task_name]["mlm_random"]# default 0.5
    mlm_probability = task_info[task_name]["mlm_probability"]#args.word_mask_probability[task_index]
...
def mask_tokens_PRP_WWM(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word_to_phrase_, token_to_phrase,token_to_word, mlm_list, args) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. """
    mlm_mask = task_info[task_name]["mlm_mask"]# default 0.8
    mlm_random = task_info[task_name]["mlm_random"]# default 0.5
    mlm_probability = task_info[task_name]["mlm_probability"]#args.word_mask_probability[task_index]
...
def mask_tokens_PRP_TokenWeight(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word_to_phrase_, token_to_phrase,token_to_word, mlm_list, args) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original. """
    mlm_mask = task_info[task_name]["mlm_mask"]# default 0.8
    mlm_random = task_info[task_name]["mlm_random"]# default 0.5
    mlm_probability = task_info[task_name]["mlm_probability"]#args.word_mask_probability[task_index]
...
def run_batch(model, batch, tokenizer, args, task_name, try_again=True):
    ...
    for sent in batch:
        ...
        #     print("Good!")
        try:
            if task_name=="PhraseWeight":
                input_, label = mask_tokens_PRP(sent_, tokenizer, task_info, task_name, token_to_word_to_phrase, token_to_phrase, token_to_word, mlm_list, wordscores, phrase_scores,args)# if args.mlm else (batch, batch)
            elif task_name=="WordWeight":
                input_, label = mask_tokens_PRP_WordWeight(sent_, tokenizer, task_info, task_name, token_to_word_to_phrase, token_to_phrase, token_to_word, mlm_list, wordscores, args)# if args.mlm else (batch, batch)
            elif task_name=="PhrasePMI":
                input_, label = mask_tokens_PRP_PMI(sent_, tokenizer, task_info, task_name, token_to_word_to_phrase, token_to_phrase, token_to_word, mlm_list, args)# if args.mlm else (batch, batch)
            elif task_name=="WholeWordMasking":
                input_, label = mask_tokens_PRP_WWM(sent_, tokenizer, task_info, task_name, token_to_word_to_phrase, token_to_phrase, token_to_word, mlm_list, args)# if args.mlm else (batch, batch)
            elif task_name=="TokenWeight":
                input_, label = mask_tokens_PRP_TokenWeight(sent_, tokenizer, task_info, task_name, token_to_word_to_phrase, token_to_phrase, token_to_word, mlm_list, args)# if args.mlm else (batch, batch)
            elif task_name=="MASKTOKENS":
