...
def mask_tokens_PRP(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, task_info, task_name, token_to_word, token_to_phrase, args) -> Tuple[torch.Tensor, torch.Tensor]:
...
def run_batch(model, batch, tokenizer, args, task_name, try_again=True):
    ...
    for sent in batch:
        ...
        # print("text")
        # print(text)
        mlm_list = mlm_model.token_score(text, PLL_metric='within_word_l2r')
        # print("score")
        # print(mlm_list)
        ...
        sent_ = torch.tensor(tokenizer(doc)["input_ids"])
        # print(sent_)

        # if "word" in task_name:
        task_info = {}

        for auxtask, mask, wmask, pmask in zip(args.aux_task_names, args.mlm_probability, args.word_mask_probability, args.phrase_mask_probability):
            task_info[auxtask] = {}
        ...
        input_, label = mask_tokens_PRP(sent_, tokenizer, task_info, task_name, token_to_word, token_to_phrase, args)# if args.mlm else (batch, batch)
